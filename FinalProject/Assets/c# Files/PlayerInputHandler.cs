...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Gun reload code
                if(Input.GetKeyDown(KeyCode.E)) { //checks for ammo and fires if there's a remaining bullet
                    Debug.Log("Firing Primary");
                    if(Remaining != 0){
            Remaining -= 1;
            projectileThrower.Launch(Camera.main.ScreenToWorldPoint(Input.mousePosition));

                } else if(Remaining == 0) //Prints "Out of Ammo" if there's no remaining bullets
                {
                    Debug.Log("Out of Ammo");
                }
                if(Input.GetKeyDown(KeyCode.R)) { // Reloads single bullet
                    Debug.Log("Loading single bullet");
                    Remaining += 1;
                }
    }
